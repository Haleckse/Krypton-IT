...
void dechiffrage_char(char* crypted, char* key, int size, char* decrypted){
    ...
    for (int i=0; i<size; i++){
        decrypted[i] = (crypted[i] ^ key[i%strlen(key)]);
    }
}
...
int chiffrage_char_fichier(char* nom_fichier, char* key){
    ...
    int fichier_msg = open(nom_fichier, O_RDONLY);
    if(fichier_msg == -1){
    ...
    int fichier_crypt = open("crypte.txt", O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IROTH);
    if(fichier_crypt == -1){
    ...
    while((nbLus == nbEcrit) && (nbLus = read(fichier_msg, bloc, TAILLE_BLOC))>0){
        chiffrage_char(bloc, key, nbLus, bloc_crypt);
...
int dechiffrage_char_fichier(char* key){
    ...
    int fichier_a_decrypt = open("crypte.txt", O_RDONLY);
    ...
    while((nbLus == nbEcrit) && (nbLus = read(fichier_a_decrypt, bloc2, TAILLE_BLOC))>0){
        dechiffrage_char(bloc2, key, nbLus, bloc_decrypt);
